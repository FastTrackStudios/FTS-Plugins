...
mod platform {
    ...
    fn validate_frame_pointer_chain(min_depth: usize) -> Result<(), String> {
        ...
        // On NixOS, glibc is not compiled with -fno-omit-frame-pointer so the
        // chain breaks early (typically at depth 2). Rather than panicking, we
        // accept whatever depth we got — trace quality will be lower but the
        // process can continue.
        if depth < min_depth {
            eprintln!(
                "moire: frame pointer chain shallow ({depth}/{min_depth}) — \
                 traces may be incomplete (expected on NixOS)"
            );
        }
